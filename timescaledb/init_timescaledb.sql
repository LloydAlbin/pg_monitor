...
CREATE INDEX postgres_log_idx ON logs.postgres_log USING btree (log_time, cluster_name, database_name);
CREATE INDEX postgres_log_pkey ON logs.postgres_log USING btree (cluster_name, session_id, session_line_num);
   ...
   FROM ((_timescaledb_catalog.hypertable ht
     ...
     LEFT JOIN LATERAL public.hypertable_relation_size((
        CASE
            ...
            ELSE NULL::text
        END)) size(table_size, index_size, toast_size, total_size) ON (true));
...
CREATE OR REPLACE FUNCTION tools.postgres_log_trigger() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
BEGIN
...
CREATE OR REPLACE FUNCTION tools.create_stats() RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  queries RECORD;
...
END;
$$;
ALTER FUNCTION tools.create_stats() OWNER TO grafana;

CREATE OR REPLACE FUNCTION tools.create_server_database_inherits("server_name" text, database_name text) RETURNS void
    LANGUAGE plpgsql
    AS $_$
DECLARE
  tables RECORD;
...
CREATE OR REPLACE FUNCTION tools.create_server_inherits("server_name" text) RETURNS void
    LANGUAGE plpgsql
    AS $_$
DECLARE
  tables RECORD;
...
CREATE OR REPLACE FUNCTION tools.delete_stats() RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  tables RECORD;
...
BEGIN
  ...
  PERFORM tools.create_stats();

END;
$$;
ALTER FUNCTION tools.delete_stats() OWNER TO grafana;
...
CREATE OR REPLACE FUNCTION tools.generate_timestamps("interval" text, "between" text) RETURNS TABLE(start_time timestamp with time zone, end_time timestamp with time zone)
...
CREATE OR REPLACE FUNCTION tools.group_by_interval(grafana_interval text, "interval" text) RETURNS text
...
CREATE OR REPLACE FUNCTION tools.interval_to_field(grafana_interval text) RETURNS text
    LANGUAGE sql STRICT
    AS $_$
...
CREATE OR REPLACE FUNCTION tools.parse_csv(s text, raise_on_error boolean DEFAULT true) RETURNS text[]
...
CREATE OR REPLACE FUNCTION logs.autoanalyze_log(grafana_time_filter text, cluster_name_in text DEFAULT NULL::text, database_name_in text DEFAULT NULL::text, schema_name_in text DEFAULT NULL::text, table_name_in text DEFAULT NULL::text, query_limit bigint DEFAULT 100000) RETURNS TABLE("time" timestamp with time zone, cluster_name text, database_name text, schema_name text, table_name text, cpu_system numeric, cpu_user numeric, elasped_seconds numeric)
    LANGUAGE plpgsql
    AS $_X$
/*
...
CREATE OR REPLACE FUNCTION logs.autoanalyze_log_count_chart(grafana_time_filter text, cluster_name_in text, database_name_in text, schema_name_in text, table_name_in text) RETURNS TABLE("time" timestamp with time zone, table_name text, count bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
SET application_name = 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.autovacuum_autoanalyze_count(grafana_time_filter text, cluster_name_in text DEFAULT NULL::text, database_name_in text DEFAULT NULL::text, schema_name_in text DEFAULT NULL::text, table_name_in text DEFAULT NULL::text) RETURNS TABLE("time" timestamp with time zone, vacuum bigint, "analyze" bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
SET application_name TO 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.autovacuum_log_count_chart(grafana_time_filter text, cluster_name_in text, database_name_in text, schema_name_in text, table_name_in text) RETURNS TABLE("time" timestamp with time zone, table_name text, count bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
SET application_name = 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.autovacuum_log_removed_size(grafana_interval text, grafana_time_filter text, cluster_name_in text, database_name_in text, schema_name_in text, table_name_in text) RETURNS TABLE("time" timestamp with time zone, cluster_name text, removed_size bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
SET application_name = 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.autovacuum_log_removed_space_chart(grafana_time_filter text, cluster_name_in text, database_name_in text, schema_name_in text, table_name_in text) RETURNS TABLE("time" timestamp with time zone, table_name text, removed_size bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
SET application_name = 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.autovacuum_log_tuples_removed(grafana_interval text, grafana_time_filter text, cluster_name_in text, database_name_in text, schema_name_in text, table_name_in text) RETURNS TABLE("time" timestamp with time zone, cluster_name text, tuples_removed bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
SET application_name = 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.autovacuum_log_tuples_removed_chart(grafana_time_filter text, cluster_name_in text, database_name_in text, schema_name_in text, table_name_in text) RETURNS TABLE("time" timestamp with time zone, table_name text, tuples_removed bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
SET application_name = 'Grafana';
...
FROM (
	...
	FROM stats.autovacuum_thresholds
    WHERE 
...
LEFT JOIN stats.autovacuum_thresholds b USING (log_time, cluster_name, database_name)
...
CREATE OR REPLACE FUNCTION logs.checkpoint_files(grafana_interval text, grafana_time_filter text, grafana_from_time timestamp with time zone, grafana_to_time timestamp with time zone, cluster_name_in text DEFAULT NULL::text) RETURNS TABLE("time" timestamp with time zone, cluster_name text, files_added bigint, files_removed bigint, files_recycled bigint, sync_files bigint, sync_longest numeric, sync_avg numeric)
    LANGUAGE plpgsql
    AS $_X$
/*
SET application_name TO 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.checkpoint_wal_file_usage(grafana_interval text, grafana_time_filter text, grafana_from_time timestamp with time zone, grafana_to_time timestamp with time zone, cluster_name_in text DEFAULT NULL::text) RETURNS TABLE("time" timestamp with time zone, cluster_name text, files bigint)
    LANGUAGE plpgsql
    AS $_X$
/*
SET application_name TO 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.checkpoint_warning_logs(grafana_time_filter text, cluster_name_in text DEFAULT NULL::text, query_limit bigint DEFAULT 100000) RETURNS TABLE("time" timestamp with time zone, cluster_name text, seconds integer, hint text)
    LANGUAGE plpgsql
    AS $_X$
/*
SET application_name TO 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.checkpoint_warning_logs_count(grafana_interval text, grafana_time_filter text, grafana_from_time timestamp with time zone, grafana_to_time timestamp with time zone, cluster_name_in text DEFAULT NULL::text) RETURNS TABLE("time" timestamp with time zone, cluster_name text, count bigint)
    LANGUAGE plpgsql
    AS $_X$
/*
SET application_name TO 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.checkpoint_write_buffers(grafana_interval text, grafana_time_filter text, grafana_from_time timestamp with time zone, grafana_to_time timestamp with time zone, cluster_name_in text DEFAULT NULL::text) RETURNS TABLE("time" timestamp with time zone, cluster_name text, wbuffer bigint)
    LANGUAGE plpgsql
    AS $_X$
/*
SET application_name TO 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.connection_attempt_history(grafana_interval text, grafana_time_filter text, cluster_name text[] DEFAULT '{''All''::text}'::text[], "interval" text DEFAULT '1s'::text, aggregate text DEFAULT 'avg'::text, display_interval boolean DEFAULT false, display_aggregate boolean DEFAULT false) RETURNS TABLE("time" timestamp with time zone, "Server" text, "Connections" bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
SET application_name = 'Grafana';
...
CREATE OR REPLACE FUNCTION logs.custom_table_settings(server_name text, db_name text, grafana_timeto timestamp with time zone, grafana_refresh text) RETURNS TABLE("time" timestamp with time zone, cluster_name text, database_name name, table_name text, table_setting text)
    LANGUAGE plpgsql STRICT
    AS $_X$
/*
Usage:
...
FROM (
	SELECT max(log_time) AS log_time, cluster_name, database_name
	FROM stats.custom_table_settings
...
LEFT JOIN stats.custom_table_settings b USING (log_time, cluster_name, database_name)
...
CREATE OR REPLACE FUNCTION logs.error_history(grafana_interval text, grafana_time_filter text, cluster_name text[] DEFAULT '{''All''::text}'::text[], "interval" text DEFAULT 'second'::text, aggregate text DEFAULT 'avg'::text, display_interval boolean DEFAULT false) RETURNS TABLE("time" timestamp with time zone, "LDAP Errors" text, "Errors" bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
...
CREATE OR REPLACE FUNCTION logs.fatal_history(grafana_interval text, grafana_time_filter text, cluster_name text[] DEFAULT '{''All''::text}'::text[], "interval" text DEFAULT 'second'::text, aggregate text DEFAULT 'avg'::text, display_interval boolean DEFAULT false) RETURNS TABLE("time" timestamp with time zone, "LDAP Errors" text, "Errors" bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
-- THIS FUNCTION HAS NOT BEEN FINISHED WRITTEN
/*
...
CREATE OR REPLACE FUNCTION logs.ldap_error_history(grafana_interval text, grafana_time_filter text, cluster_name text[] DEFAULT '{''All''::text}'::text[], "interval" text DEFAULT '1s'::text, aggregate text DEFAULT 'avg'::text, display_interval boolean DEFAULT false) RETURNS TABLE("time" timestamp with time zone, "LDAP Errors" text, "Errors" bigint)
    LANGUAGE plpgsql STRICT
    AS $_X$
-- THIS FUNCTION HAS NOT BEEN FINISHED WRITTEN
/*
SET application_name = 'Grafana';
SELECT * FROM logs.ldap_error_history('$__interval', $$$__timeFilter(log_time)$$, ARRAY[$ServerName]::text[], '1m', 'sum', False);
SELECT * FROM logs.ldap_error_history('$__interval', $$$__timeFilter(log_time)$$, ARRAY[$ServerName]::text[], '$__interval', 'sum', True);

aka

SELECT * FROM logs.ldap_error_history('5s', $$log_time BETWEEN '2019-03-11T19:45:08Z' AND '2019-03-11T22:45:08Z'$$, ARRAY['sqltest'], '1s', 'avg', False);
...
*/
  IF grafana_interval::interval = interval::interval OR aggregate ILIKE 'sum' THEN
  	sql := E'SELECT time_bucket_gapfill(CASE WHEN ''' || grafana_interval || E'''::interval >= ''' || interval || E'''::interval THEN ''' || grafana_interval || E'''::interval ELSE ''' || interval || E'''::interval END, log_time) AS "Time", ';
    sql = sql || E'CASE WHEN array_length(''' || cluster_name::text || E'''::text[], 1) > 1 THEN cluster_name || '' - '' ELSE '''' END || CASE WHEN message LIKE E''%Can''''t contact LDAP server'' THEN trim(split_part(message, ''"'', 1)) ELSE trim(split_part(message, '':'', 2)) END AS ldap_error, ';
    sql = sql || 'COALESCE(count(*),0) AS "Errors" ';
    sql = sql || 'FROM logs.postgres_log ';
    sql = sql || E'WHERE ARRAY[cluster_name] <@ ''' || cluster_name::text || E'''::text[] AND (message LIKE ''LDAP login failed for user%'' OR message LIKE E''%Can''''t contact LDAP server'') AND ' || grafana_time_filter || ' ';
    sql = sql || 'GROUP BY "Time",ldap_error ORDER BY "Time",ldap_error ';
  ELSE
	  sql := E'SELECT time_bucket(CASE WHEN ''' || grafana_interval || E'''::interval >= ''' || interval || E'''::interval THEN ''' || grafana_interval || E'''::interval ELSE ''' || interval || E'''::interval END, "Time1") AS "Time", ldap_error, COALESCE(' || aggregate || '("Errors1"),0)::BIGINT AS "Errors" FROM ( ';
  	sql = sql || E'SELECT time_bucket_gapfill(''' || interval || ''', log_time) AS "Time1", ';
    sql = sql || E'CASE WHEN array_length(''' || cluster_name::text || E'''::text[], 1) > 1 THEN cluster_name || '' - '' ELSE '''' END || CASE WHEN message LIKE E''%Can''''t contact LDAP server'' THEN trim(split_part(message, ''"'', 1)) ELSE trim(split_part(message, '':'', 2)) END AS ldap_error, ';
    sql = sql || 'count(*) AS "Errors1" ';
    sql = sql || 'FROM logs.postgres_log ';
    sql = sql || E'WHERE ARRAY[cluster_name] <@ ''' || cluster_name::text || E'''::text[] AND (message LIKE ''LDAP login failed for user%'' OR message LIKE E''%Can''''t contact LDAP server'') AND ' || grafana_time_filter || ' ';
...
CREATE OR REPLACE FUNCTION logs.update_pg_log_databases() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
--  variable_name datatype;
...
CREATE TABLE tools.hypertables (
    ...
    continous_agg_bucket_width TEXT[],
    continous_agg_refresh_lag TEXT[],
    continous_agg_refresh_interval TEXT[],
    continous_agg_max_interval_per_job TEXT[],
    continous_agg_create_group_indexes BOOLEAN[],
    continous_agg_ignore_invalidation_older TEXT,
    continous_agg_aggregate TEXT,
    continous_agg_view_aggregate TEXT,
    continous_agg_additional_fields TEXT
);
...
INSERT INTO tools.hypertables (schema_name, table_name, time_column_name, partitioning_column, hash_partitions, chunk_time_interval, drop_chunk_policy, compress_chunk_policy, compress_orderby, compress_segmentby, continous_agg_bucket_width, continous_agg_refresh_lag, continous_agg_refresh_interval, continous_agg_max_interval_per_job, continous_agg_aggregate, continous_agg_view_aggregate, continous_agg_additional_fields) VALUES
('logs', 'archive_failure_log',           'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name'),
('logs', 'autoanalyze_logs',              'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name'),
('logs', 'autovacuum_logs',               'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name'),
('logs', 'checkpoint_logs',               'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name'),
('logs', 'checkpoint_warning_logs',       'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name'),
('logs', 'lock_logs',                     'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name'),
('logs', 'postgres_log',                  'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name'),
('logs', 'connection_received_logs',      'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name, connection_from', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name, connection_from'),
('logs', 'connection_authorized_logs',    'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name, connection_from, database_name, user_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name, connection_from, database_name, user_name'),
('logs', 'connection_disconnection_logs', 'log_time', 'cluster_name', 20, INTERVAL '1 week', INTERVAL '1 year', INTERVAL '1 month', 'log_time DESC', 'cluster_name, connection_from, database_name, user_name', '{"1s", "1m", "1h", "1d", "1w", "1mon", "1y"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', '{"5s", "2m", "5m", "5m", "5m", "5m", "5m"}', '{"1d", "1d", "1d", "1d", "1w", NULL, NULL}', 'count(*)', 'sum(value)', 'cluster_name, connection_from, database_name, user_name, application_name');
